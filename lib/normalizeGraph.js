...
function transformLinks(links, nodes) {
  ...
  const filteredLinks = d3
    .nest()
    .key(d => `${d[0]}--LINK_ID--${d[1]}`)
    .rollup(d => d[0]) // only keep first
    .entries(links)
    .map(linkEntry => linkEntry.value);

  if (filteredLinks.length !== links.length) {
    console.warn(
      `Warning: There are ${links.length -
        filteredLinks.length} duplicate links.`
    );
  }

  let transformedLinks = filteredLinks
    .map(link => {
  ...
  if (missingNodes.length) {
    ...
    console.warn(
      'Warning: Found links for nodes that are not in the dataset. Removing all links with node IDs: ' +
        missingNodes.join(', ')
    );
  }
